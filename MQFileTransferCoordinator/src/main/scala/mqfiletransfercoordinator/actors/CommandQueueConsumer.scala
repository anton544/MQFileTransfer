...
class CommandQueueConsumer(commandQueueName: String, agentCoordinator: ActorRef, transferCoordinator: ActorRef) extends Actor with Consumer with ActorLogging {
  def endpointUri = commandQueueName
  
  def receive = LoggingReceive {
    case camelMessage: CamelMessage => {
      log.debug("receiving message: " + camelMessage.bodyAs[String])
    ...
    case commandMessage: CommandMessage => {
      commandMessage.command match {
        ...
        case "TransferProgress" => transferCoordinator ! new TransferProgress(commandMessage.transferid, commandMessage.segmentNumber, commandMessage.segmentsTotal) 
    ...
    case _ => log.warning("Unknown message received by CommandQueueConsumer")
  }
