...
with ImplicitSender with WordSpecLike with BeforeAndAfterAll {
	...
	"An AgentTransferCoordinator receiving a StartTransferAck with a status of Success message" must {
		"send a TransferNextSegment message with a next segment number of 1 to the FileActor" in {
			AgentTransferCoordinator.pathMap.clear
			AgentTransferCoordinator.pathMap += ("1234" -> "/somefile")
			val fileActorProbe = TestProbe()
			val dataProducerProbe = TestProbe()
			...
			val coordProducerProbe = TestProbe()
			val actor = system.actorOf(Props(new AgentTransferCoordinator(dataProducerProbe.ref, cmdProducerProbe.ref, fileActorProbe.ref, coordProducerProbe.ref)))
			actor ! new CommandMessage(<message><type>StartTransferAck</type><transferid>1234</transferid><status>Success</status></message>)
			fileActorProbe.expectMsg(100 millis, TransferNextSegment("1234", "/somefile", 1))
		}
	}

	"An AgentTransferCoordinator receiving a StartTransferAck with a status of Failure message" must {
		"send a RemoveProducer message to the dataProducer" in {
			AgentTransferCoordinator.pathMap.clear
			AgentTransferCoordinator.pathMap += ("1234" -> "/somefile")
			val fileActorProbe = TestProbe()
			val dataProducerProbe = TestProbe()
			...
			val coordProducerProbe = TestProbe()
			val actor = system.actorOf(Props(new AgentTransferCoordinator(dataProducerProbe.ref, cmdProducerProbe.ref, fileActorProbe.ref, coordProducerProbe.ref)))
			actor ! new CommandMessage(<message><type>StartTransferAck</type><transferid>1234</transferid><status>Failure</status></message>)
		...
		"send a TransferFailed message to the CoordinatorProducer" in {
			AgentTransferCoordinator.pathMap.clear
			AgentTransferCoordinator.pathMap += ("1234" -> "/somefile")
			val fileActorProbe = TestProbe()
			val dataProducerProbe = TestProbe()
	...
	"An AgentTransferCoordinator receiving an InitiateTransfer message" must {
		"send a FileReadVerify message to the FileActor" in {
			...
			fileActorProbe.expectMsg(100 millis, FileReadVerify("1234", "/somefile", "/somefile2", "SOURCE.COMMAND.QUEUE", "SOURCE.DATA.QUEUE", "TARGET.COMMAND.QUEUE", "TARGET.DATA.QUEUE"))
		}
	}
	...
	"An AgentTransferCoordinator receiving a FileReadSuccess message" must {
		"send a AddProducer message to the CommandQueueProducer" in {
			...
			val coordProducerProbe = TestProbe()
			val actor = system.actorOf(Props(new AgentTransferCoordinator(dataProducerProbe.ref, cmdProducerProbe.ref, fileActorProbe.ref, coordProducerProbe.ref)))
			actor ! FileReadSuccess("1234", "/afile", "/somefile", "SOURCE.COMMAND.QUEUE", "SOURCE.DATA.QUEUE", "TARGET.COMMAND.QUEUE", "TARGET.DATA.QUEUE")
...
object AgentTransferCoordinatorSpec {
	...
	val startTransferMessage = new CommandMessage(<message><type>StartTransfer</type><transferid>1234</transferid><targetpath>/somefile</targetpath><sourcecommandqueue>SOURCE.COMMAND.QUEUE</sourcecommandqueue><sourcedataqueue>SOURCE.DATA.QUEUE</sourcedataqueue></message>)
	...
	val initiateTransfer = new CommandMessage(
		<message>
			...
			<sourcecommandqueue>SOURCE.COMMAND.QUEUE</sourcecommandqueue>
			<sourcedataqueue>SOURCE.DATA.QUEUE</sourcedataqueue>
