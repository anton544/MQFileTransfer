...
case class TransferProgress(transferid: String, segmentsComplete: Int, segmentsTotal: Int) {
	def toXmlString() = s"<message><type>TransferProgress</type><transferid>${transferid}</transferid><segmentsComplete>${segmentsComplete}</segmentsComplete><segmentsTotal>${segmentsTotal}</segmentsTotal></message>"
}
